...
const Categories = () => {
  ...
  return (
    <div className="mt-2 overflow-hidden flex justify-center">
      <div className="flex space-x-6 p-6 max-w-[1600px] overflow-x-auto">
        {categories.map((category, index) => (
          <div key={index} className="flex flex-col items-center">
            <Link to={`/category/${category.name}`} className="flex flex-col items-center">
              <div className="w-24 h-24 rounded-full overflow-hidden border-2 border-gray-300 flex items-center justify-center bg-white">
                <img
              ...
              <p className="mt-2 text-center text-gray-700 font-medium">{category.name}</p>
            </Link>
          </div>
        ))}
      </div>
