...
const AdminProducts = () => {
  ...
  if (loading) return <div className="flex justify-center items-center min-h-[60vh]"><p className="text-gray-500 text-lg">Loading products...</p></div>;
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-md p-6 mb-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={form.id} onChange={(e) => setForm({ ...form, id: e.target.value })}
                placeholder="e.g. mouse-007" className="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-500"
                disabled={!!editingProduct} />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Name *</label>
              <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
                required className="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-500" />
            </div>
            <div>
            ...
            <div className="sm:col-span-2">
              ...
              <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })}
                required rows="3" className="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-purple-500" />
            </div>
          </div>
          <div className="mt-4 flex gap-3">
            ...
            <button type="button" onClick={resetForm} className="bg-gray-200 hover:bg-gray-300 text-gray-700 px-6 py-2 rounded-lg font-medium transition">
              Cancel
            </button>
          </div>
        </form>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-gray-50 border-b">
                ...
                <th className="py-3 px-4 text-sm font-semibold text-gray-600">Brand</th>
                <th className="py-3 px-4 text-sm font-semibold text-gray-600">Category</th>
                <th className="py-3 px-4 text-sm font-semibold text-gray-600">Price (D/W/M)</th>
                <th className="py-3 px-4 text-sm font-semibold text-gray-600">Security</th>
